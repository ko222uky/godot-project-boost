...
func _process(delta: float) -> void:
	if Input.is_action_pressed("ui_cancel"): # For exiting the game client
		get_tree().quit()
...
func crash_sequence() -> void:
	print("KABOOM!")
	# After crashing, stop any engine audio and particles
	engine_thrust_audio.stop()
	small_engine_thrust_audio.stop()
	booster_particles.emitting = false
	booster_particles_left.emitting = false
	...
	# play one-shot explosion particles and audio
